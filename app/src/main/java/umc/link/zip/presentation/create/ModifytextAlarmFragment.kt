...
@AndroidEntryPoint
class ModifytextAlarmFragment :
    BaseFragment<FragmentCustomtextAlarmBinding>(R.layout.fragment_customtext_alarm),
            ...
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                linkGetByIDViewModel.linkGetByLinkIDResponse.collectLatest { state ->
                    when (state) {
                        is UiState.Loading -> {
                            // 로딩 상태 처리
                            Log.d("ModifytextAlarmFragment", "Loading 썸네일")
                        }
                        ...
                        is UiState.Success<*> -> {
                            ...
                            Log.d("ModifytextAlarmFragment", "썸네일 가져오기 성공")

                        }

                        is UiState.Error -> {
                            Log.d("ModifytextAlarmFragment", "썸네일 가져오기 실패")
                        }

                        UiState.Empty -> Log.d("ModifytextAlarmFragment", "isEmpty")
                    }
                }
            }
        }
